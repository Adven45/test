...
def setup_printer(sn,url):
    ...
    #driver.find_element(By.NAME, "PRINTER_INFO").send_keys("Zebra")
   ...
   # Select(Select_make).select_by_value("Generic")
    ...
    #Select(select_model).select_by_value("lsb/usr/cupsfilters/TT033-50.ppd")
...
ADD_PRINTERS=["10.92.62.47/MKB-1F-OPD6"]
for ip in list:
    #setup_printer("10.92.62.69/MKB-2F-ICU3",ip)
    for addprint in ADD_PRINTERS:
        setup_printer(addprint,ip)
        print(addprint,"is done",ip)
